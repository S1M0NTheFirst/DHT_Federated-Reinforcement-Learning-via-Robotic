#!/bin/bash
#MSUB -N task2_dht_frl
#MSUB -W group_list=hpc2-coe-users
#MSUB -l walltime=12:00:00
#MSUB -j oe
# Submit onto 3 distinct healthy nodes with:
#   cluster/tools/submit_free.sh cluster/task2/condition_dht_frl/run.sh
# (submit_free.sh overrides the nodes= spec with 3 free confirmed-good hosts.)

set -uo pipefail
CONDITION_DIR="condition_dht_frl"
...
export RESULTS_DIR="$RESULTS_ROOT/task2_dht_frl"
mkdir -p "$RESULTS_DIR"

echo ">>> Launching task2 dht_frl runner" | tee -a "$RUNNER_LOG"
